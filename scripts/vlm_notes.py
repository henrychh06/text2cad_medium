...
def generate_shape_description(image_paths, device, processor, model):
    if not image_paths:
...
/INST]
    ...
    for img_path in image_paths:
        ...
        try:
            # Procesa la imagen y el prompt con el processor
...
def process_cad_file(uid, json_path, renders_base_dir, output_dir, device, args, processor, model):
    try:
        ...
        if not image_paths:
            ...
            return None
        shape_description = generate_shape_description(image_paths, device, processor, model)
...
def main(args):
    ...
    print("[DEBUG] Cargando processor y modelo del VLM...")
    try:
        ...
        model.to(device)
        print("[DEBUG] Processor y modelo cargados correctamente.")
    except Exception as e:
        print(f"[DEBUG] Error cargando processor/modelo: {e}")
        return
    ...
    for uid in tqdm(all_uids, desc="Generando descripciones"):
        ...
        result = process_cad_file(uid, json_path, args.renders_dir, args.output_dir, device, args, processor, model)
        if result:
